...
| **`plano-instalacion` — filtro de obras acotado a ACEPTADO/EN_OBRA (21/09/2026)** | `RELEVANT_ESTADOS` pasó de `['ACEPTADO', 'EN_OBRA', 'OBRA_ENTREGADA', 'FACTURADO', 'CERRADO']` a `['ACEPTADO', 'EN_OBRA']` — antes se veían mezcladas 12 obras `CERRADO` (ninguna con plano cargado) con las 4 confirmadas/activas reales, metiendo ruido. Motivo: el módulo es para instalación en curso, no archivo histórico. Verificado antes del cambio contra `vvwnyszcfindtuvojqgs`: ninguna obra en `OBRA_ENTREGADA`, `FACTURADO` o `CERRADO` tenía filas en `proyectos_planos_instalacion` — el cambio no oculta ningún plano existente. `ESTADO_PILL` queda con entradas sin uso (no rompe nada) |
