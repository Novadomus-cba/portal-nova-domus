...
| **Numeración certificado/recibo compartida (23/09/2026)** | Problema: `certificados.numero` y `recibos.numero` se calculaban con dos `MAX(numero)+1` independientes por tabla. Coincidían solo mientras cada certificado generaba exactamente un recibo en el mismo paso; un recibo BORRADOR huérfano (wizard abandonado) rompía esa coincidencia para siempre a partir de ahí. Casos reales encontrados: CH3 (recibo BORRADOR huérfano de hoy, con línea de material cargada — no se tocó), LBV (recibo BORRADOR huérfano sin datos — se borró), CPC (recibo #991 vs certificado #19, anomalía histórica anterior al fix del 21/08 de Zanoli/Los Boulevares — se deja como está, Agustín decidió no tocarlo). **Fix:** RPC `fn_next_certificado_recibo_numero(obra_id)` — número compartido único entre las dos tablas, con advisory lock por obra (permiso solo `authenticated`, `anon` revocado). `obras/index.html` ahora pide un solo número y lo usa para certificado y recibo (`onSObraChange()` para el preview, `ensureRecibo()` para el valor real al entrar al paso 3, `generar()` lee `sNumC`/`sNumR` ya fijados en vez de recalcular). **Constraint nueva** en `recibos`: `recibo_certificado_si_no_borrador` — `certificado_id` obligatorio salvo `estado = 'BORRADOR'` (decisión de Agustín: no deberían existir recibos sin certificado, salvo el hueco transitorio inevitable del wizard mientras arma el certificado) |
| **`plano-instalacion` — vigente/obsoleto, RPC atómica, avance de relevamiento (22/09/2026)** | Handoff del Project de Obra y Ejecución (v1.0). `RELEVANT_ESTADOS` **queda en `['ACEPTADO','EN_OBRA']`** — mismo valor que ya tenía desde el 21/09/2026. Durante este mismo trabajo se probó ampliarlo a `['ACEPTADO','EN_OBRA','OBRA_ENTREGADA','FACTURADO','CERRADO']` (para que el avance de relevamiento nuevo también se viera en obras terminadas), pero Agustín lo revirtió al toque: prefiere que el listado muestre solo obras confirmadas y en curso, no un archivo histórico. La sección de avance de relevamiento (ver más abajo) queda igual en el código, solo que ahora únicamente es visible mientras la obra está en `ACEPTADO`/`EN_OBRA`. **Objetos nuevos en Supabase** (agregados antes de tocar el frontend, `vvwnyszcfindtuvojqgs`): `fn_subir_plano_instalacion(...)` (RPC `SECURITY INVOKER`, apaga el plano vigente anterior y prende el nuevo en una transacción, `version = MAX+1` por `codigo`, `anon` sin `EXECUTE`) reemplaza el `INSERT` directo; `v_planos_instalacion_listado` (liviana, `tiene_html`/`tiene_storage` en vez del `html` completo, `anon` sin `SELECT`) reemplaza la consulta directa a `proyectos_planos_instalacion` — el listado ahora muestra la versión **vigente**, no la de número más alto (bug real confirmado: PI-2026-019 tenía v3 obsoleta — el plano de El Timbo cargado por error en la carpeta de Zanoli — con versión mayor a v2, la vigente); `v_planos_relevamiento_avance` (mismos permisos) alimenta la sección nueva. **Guardarraíl agregado a la subida manual:** el `<title>` del HTML debe incluir el número del proyecto destino o se corta antes de subir a Storage — sin esto pasó lo de PY-2026-019/v3 (confirmado byte a byte contra el archivo real de El Timbo). **Decisión de arquitectura — sesión del plano interactivo:** se descartó pasar el token por `postMessage`; `verPlano()` sigue abriendo el plano como Blob en pestaña nueva (mecanismo que ya existía para esquivar el Content-Type anti-XSS de las signed URLs de Storage) — un Blob hereda el origen de quien lo crea, así que esa pestaña queda en el origen del portal y comparte `localStorage`, y cualquier cliente Supabase que el propio plano instancie encuentra la sesión activa sola. Diagnóstico en consola agregado para confirmarlo en el primer uso real (`[ND diagnóstico sesión] SESION OK: <email>` / `SIN SESION`); sacarlo una vez que el Project de Obra tenga planos interactivos reales probados en producción — **verificación de esta hipótesis pendiente de que Agustín la corra en el portal real, no confirmada todavía**. **Incidente de deploy (mismo día):** el primer commit de este script de diagnóstico dejó un `</script>` literal sin escapar dentro del template literal — el parser HTML corta el `<script>` inline ahí mismo, antes de que corra ningún JS, así que `init()` nunca se ejecutaba y el módulo quedaba colgado en "Verificando acceso..." (el texto crudo del archivo seguía intacto, por eso no lo detectó un `curl` de verificación). Corregido escapando como `<\/script>` y verificado que no queda ningún cierre prematuro en el bloque de script antes del cierre real. **No incluido en este trabajo** (Project de Obra o Agustín, según el handoff): borrar `PY-2026-019/v3.html` de Storage, decidir sobre `PY-2026-007/v1.html` huérfano, migrar a `html` inline los planos que hoy están solo en Storage |
